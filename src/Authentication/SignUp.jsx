...
import { auth, signInWithGooglePopup, signUpWithEmail } from "../firebase";
...
const SignUp= () => {
  ...
  const handleEmailSignUp = (event) => {
    console.log(email,password,"hello password and email");
  ...
  const handleGoogleSignUp = async () => {
    try {
      ...
      if (domain !== 'ienetworks.com') {
        await auth.signOut();
        setError('You must sign up with a company email (ienetworks.com).');
      } else {
        console.log('User signed up with Google:', userCredential.user);
        // You may optionally handle additional user creation logic here
      }
    } catch (error) {
      console.error('Error signing up with Google:', error);
      setError(error.message); // Ensure setError is defined in your component or context
    }
  };
              ...
              <form onSubmit={handleEmailSignUp}>
                ...
                <div className="mb-5">
                  <input
                    ...
                    className="w-full cursor-pointer rounded-lg border border-primary bg-primary p-4 text-white bg-indigo-600 transition hover:bg-opacity-90"
                  />
                </div>

                <button type='button' onClick={handleGoogleSignUp} className="flex w-full items-center justify-center gap-3.5 rounded-lg border border-stroke bg-gray p-4 hover:bg-opacity-50 dark:border-strokedark dark:bg-meta-4 dark:hover:bg-opacity-50">
                  <span>
                    <svg
                      width="20"
                      height="20"
